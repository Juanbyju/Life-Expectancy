...
[\hyperref[Figure 2.16]{Figure 2.16}] 

[\hyperref[Figure 2.17]{Figure 2.17}] 
...
**1. Rahul Gandhi**:  
- Conducted multi-collinearity testing using Variance Inflation Factor (VIF) and scatterplot matrices to identify and remove highly correlated terms.  
- Ensured model stability by addressing potential multicollinearity issues.  
- Performed a detailed residual analysis to evaluate key regression assumptions, including linearity, independence, equal variance, and normality.  

**2. Ojas Sharma**:  
- Focused on developing and testing the initial regression models.  
- Conducted a full F-test to evaluate the overall significance of the model.  
- Performed individual t-tests to identify and remove non-significant terms.  
- Utilized stepwise regression to finalize the selection of significant predictors.  

**3. Juan Byju**:  
- Designed and created visualizations to illustrate data trends and model outputs.  
- Defined the project’s motivation and objectives.  
- Refined the model by implementing stepwise regression, comparing significant terms, and evaluating models using metrics such as BIC, AIC, and Mallows’ Cp to ensure the inclusion of meaningful predictors.  
- Compiled and interpreted the results of the final model.  

**Ayush Senthil Nelli**:  
- Built and tested interaction models, iteratively removing insignificant terms to improve the model.  
- Validated the model by ensuring residual assumptions were met.  
- Leveraged the GGally package to identify and assess highly correlated terms for significance.  
- Introduced higher-order terms, making adjustments only when they significantly enhanced the adjusted \( R^2 \).  

**Sri Harsha Tuttaganti**:  
- Focused on higher-order model development, adding and testing higher-order terms iteratively.  
- Evaluated the significance of these terms and finalized the optimal model structure.  
- Ensured that all retained terms satisfied both statistical and practical significance criteria in the final model.  

**Justification for Equity in Workload Distribution**

The workload distribution was equitable despite the variation in specific roles and tasks. Each member’s responsibilities were assigned based on their strengths, ensuring efficiency and high-quality outputs. The effort and time required for each task were balanced, preventing any individual from being overburdened. Collaborative efforts, such as model refinement and group discussions, allowed for shared contributions and active participation from all team members. This approach not only maintained fairness but also fostered teamwork, effectively utilizing the strengths of each member to achieve the project’s objectives.  
...
columns <- c(
  ...
  "Alcohol_consumption", "BMI", "GDP_per_capita", "Life_expectancy", "Hepatitis_B", "Measles", "BMI", "Polio", "Diphtheria", "Incidents_HIV", "GDP_per_capita", "Population_mln", "Thinness_ten_nineteen_years", "Thinness_five_nine_years", "Schooling"
)
